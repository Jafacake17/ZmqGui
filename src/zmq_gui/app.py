    ...
    def _build_page(self):
        ...
        def index():
            ...
            with ui.tab_panels(tabs, value=initial_tab).classes("w-full").style(
              ...
              # Inner sub-tabs: "Active" (live/filter view) and "Closed" (paginated history).
              if sta_book_tab is not None:
                with ui.tab_panel(sta_book_tab):
                    ...
                    with ui.tabs().props("dense").classes("mt-2") as _book_inner_tabs:
                        _book_active_tab   = ui.tab("Active")
                        _book_closed_tab   = ui.tab("Closed")
                    with ui.tab_panels(_book_inner_tabs, value=_book_active_tab).classes("w-full"):

                      # ---- inner: Active / recent view ----
                      with ui.tab_panel(_book_active_tab):
                      ...
                      # ---- inner: Closed (paginated history) ----
                      with ui.tab_panel(_book_closed_tab):
